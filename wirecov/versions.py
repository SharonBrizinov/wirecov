...
from wirecov.exceptions import GitLabApiError, VersionNotFoundError
...
def fetch_tags(refresh: bool = False, limit: int = 100) -> List[Dict]:
    ...
    Results are cached in ~/.cache/wirecov/tags.json for 1 hour.
    """
...
def list_versions(show_all: bool = False, refresh: bool = False,
    ...
    console.print(table)
    console.print()
    console.print("[dim]● = Docker image already built locally (ready to run)[/dim]")
    console.print()
...
def select_version_interactive(console: Console = None) -> str:
    ...
    # InquirerPy fuzzy expects dicts with "name" (display) and "value" (returned)
    choices = []
    display_to_value = {}
    ...
    try:
        ...
        # Build proper Choice objects for InquirerPy
        inquirer_choices = [
...
def validate_version(version: str) -> Tuple[str, bool]:
    ...
    # Commit hashes are always accepted
